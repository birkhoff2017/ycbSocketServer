package com.ycb.socket.constant;

/**
 * Created by LYSC on 2017/9/20.
 */
public class GlobalConfig {
    public static final String APP_ID="";
    public static final String APP_SECRET="";
    public static final String WX_ACCESS_TOKEN_URL="";
    public static final String WX_SEND_TEMPLATE_MESSAGE="";
    public static final String TEMPLATE_RENT_SUCCESS="";
    public static final String TEMPLATE_RENTURN_BACK_SUCCESS="";
    public static final String ALIPAY_APPID="";
    public static final String ALIPAY_PRIVATEKEY="";
    public static final String ALIPAY_FORMAT="json";
    public static final String ALIPAY_CHARSET="UTF-8";
    public static final String ALIPAY_ALIPAYPUBLICKEY="";
    public static final String ALIPAY_SIGNTYPE="";
    public static final String ALIPAY_SERVER_URL="https://openapi.alipay.com/gateway.do";
    public static final String ALIPAY_SEND_BORROW_MESSAGE="24b75e2d6fb442798d0d8565f02f9a1b";
    public static final String ALIPAY_SEND_ERROR_MESSAGE="fe6e474648a34949a053cd374cf0cfb5";
    public static final String ALIPAY_SEND_RETURN_MESSAGE="9bc0cc74b6fc4949a7fcd9736f812469";
}
